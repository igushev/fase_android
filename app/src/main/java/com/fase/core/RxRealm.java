...
public class RxRealm {
    ...
    public static <T extends RealmObject> Flowable<List<T>> listenList(final Function<Realm, RealmResults<T>> query) {
        final AtomicReference<Realm> realmReference = new AtomicReference<>(null);
        return listenRealmResults(query, realmReference)
                .map(result -> realmReference.get()
                        .copyFromRealm(result));
    }

    public static <T extends RealmObject> Flowable<T> listenElement(final Function<Realm, RealmResults<T>> query) {
        final AtomicReference<Realm> realmReference = new AtomicReference<>(null);
        return listenRealmResults(query, realmReference)
                .filter(result -> !result.isEmpty())
                .map(result -> realmReference.get()
                        .copyFromRealm(result.first()));
    }

    public static <T extends RealmObject> Maybe<List<T>> getList(final Function<Realm, RealmResults<T>> query) {
    ...
    public static <T extends RealmObject> Single<Boolean> isElementExist(final Function<Realm, T> query) {
        return Single.create(emitter -> {
            final Realm realm = Realm.getDefaultInstance();
            final T result = query.apply(realm);
            emitter.onSuccess(result != null && result.isLoaded() && result.isValid());
            emitter.setCancellable(realm::close);
        });
    }
    ...
    private static <T extends RealmObject> Flowable<RealmResults<T>> listenRealmResults(final Function<Realm, RealmResults<T>> query, AtomicReference<Realm> realmReference) {
